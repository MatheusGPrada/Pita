/* eslint-disable max-len */

const messages = {
    pt: {
        translations: {
            buttonLabels: {
                addSchedule: 'Adicionar agendamento',
                back: 'Voltar',
                changeCellphone: 'Celular',
                changeEmail: 'Email',
                changeName: 'Nome',
                changePassword: 'Senha',
                doSchedule: 'Realizar agendamento',
                email: 'Email',
                finish: 'Finalizar',
                login: 'Entrar',
                loginWithFacebook: 'Continue com Facebook',
                logout: 'Sair da conta',
                next: 'Próximo',
                seeAllSchedule: 'Ver todos',
            },
            descriptions: {
                capitalLetter: '- Uma letra maiúscula e uma minúscula;',
                minimalLetters: '- Deve conter no mínimo 8 digitos.',
                number: '- Um número;',
                specialCharacter: '- Um carácter especial;',
            },
            error: {
                emptyName: 'O nome deve ser preenchido',
                invalidCPF: 'O CPF informado é inválido',
                invalidData: 'Os Dados informados são inválidos',
                invalidDate: 'A data informada é inválida',
                invalidEmail: 'E-mail inválido',
                invalidPassword: 'A senha informada é inválida',
                invalidPhoneNumber: 'Número de Celular inválido',
                invalidUserOrPassword: 'Usuário ou senha inválidos',
            },
            labels: {
                OK: 'OK',
                birthDate: 'Data de nascimento',
                cellphone: 'Celular',
                cpf: 'CPF',
                email: 'E-mail',
                forgotPassword: 'Esqueceu a senha',
                login: 'Entrar',
                name: 'Nome',
                or: 'Ou',
                password: 'Senha',
                register: 'Cadastrar',
                user: 'Usuário',
            },
            steps: {
                email: 'E-mail',
                password: 'Senha',
                personalData: 'Dados',
                phoneNumber: 'Celular',
            },
            subtitle: {
                createSchedule: 'Agende agora mesmo o seu atendimento',
                insertEmail: 'Informe o e-mail para receber novidades e promoções',
                insertPassword: 'Sua senha deve conter:',
                insertPhoneNumber: 'Informe o seu número de celular para contato',
                signUp: 'Informe seus dados pessoais abaixo',
                validPhoneNumber: 'Será enviado um código para o seu celular',
            },
            title: {
                cellphone: 'Celular',
                changeProfileData: 'Alterar dados cadastrais',
                email: 'E-mail',
                password: 'Senha',
                schedule: 'Nenhum agendamento',
                scheduleds: 'Agendados',
                welcome: 'Bem vindo',
            },
            validation: {
                invalidCPF: 'CPF inválido!',
            },
        },
    },
}

export { messages }
